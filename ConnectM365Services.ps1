  ...
  {  
   ...
   AzureAD
   {
    ...
    If((Get-MsolUser -MaxResults 1) -ne $null)
    {
     ...
     {
      $SharePointHostName=(Get-MsolDomain).Name -split ".onmicrosoft.com"
   ...
   SharePoint
   {
    ...
    if(!($PSBoundParameters['SharePointHostName']) -and ([string]$SharePointHostName -eq "") ) 
    {
     ...
     $SharePointHostName= Read-Host "Please enter SharePoint oranization name"  
    }
   ...
   SecAndCompCenter
   {
    if($MFA.IsPresent)
    {
     ...
     #Importing Exchange MFA Module
     . "$MFAExchangeModule"
     if([string]($Services -contains "Exchange") -eq "False")
     {  
      Connect-IPPSSession
     }
     else
